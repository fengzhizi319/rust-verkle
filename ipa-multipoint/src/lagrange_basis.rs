...
impl LagrangeBasis {
    ...
    pub(crate) fn batch_invert (a: &mut Vec<Fr>) -> Vec<Fr>{

        let mut zeroes = Vec::with_capacity(a.len());
        let mut res: Vec<Fr>= Vec::with_capacity(a.len());
        for _ in 0..a.len() {
            res.push(Fr::zero());
            zeroes.push(false);
        }
        ...
        accumulator=   accumulator.inverse().unwrap();

        for i in (0..=a.len()-1).rev(){

            if zeroes[i as usize] {
                continue
            }
            res[i as usize]=res[i as usize]*accumulator;
            accumulator=accumulator*a[i as usize];
        }
        return res;
    }

    // We use this method to compute L_i(z) where z is not in the domain
    //计算L_i(z)的值，其中z不在定义域中
    pub(crate) fn evaluate_lagrange_coefficients(
        ...
        //batch_invert
        let mut lagrange_evals = Self::batch_invert(&mut lagrange_evaluations);
        for i in 0..domain_size {
            lagrange_evals[i] = lagrange_evals[i] * a_z;
        }

        //let end = std::time::Instant::now();
        //println!("batch_inversion_and_mul time time: {:?}", end.duration_since(start));
...
mod tests {
    ...
    fn basic_interpolation() {
        ...
        println!("coeff_poly0: {:?}", coeff_poly[0]);
        println!("coeff_poly1: {:?}", coeff_poly[1]);

        let got_p1 = coeff_poly.evaluate(&Fr::from(0u128));
        let got_p2 = coeff_poly.evaluate(&Fr::from(1u128));


        println!("got_p1: {:?}", got_p1);
        println!("got_p2: {:?}", got_p2);
