...
impl MultiPoint {
    ...
    pub fn open_opt(
        crs: CRS, // 公共参考字符串
        precomp: &PrecomputedWeights, // 预计算的权重
        transcript: &mut Transcript, // 可变的Transcript对象
        queries: Vec<ProverQuery>, // 包含ProverQuery的向量
    ) -> MultiPointProof { // 返回一个MultiPointProof
        // 在Transcript对象中添加一个名为"multiproof"的域分隔符
        transcript.domain_sep(b"multiproof");
        ...
        //let queries2=queries1.clone();
        //let queries=queries1.clone();
        // 1. 计算 `r`
        //let mut queries=queries1.clone();
        let chunk_size = (queries.len() + 19) / 20; // 计算每组的大小
        let query_chunks: Vec<_> = queries.chunks(chunk_size).collect();

        // 并行处理每组queries
        let hash_results: Vec<Vec<u8>> = query_chunks.par_iter().map(|chunk| {
            let mut hasher = Sha256::new();
            for query in chunk.iter() {
                let mut bytes = [0u8; 96];
                query.point.serialize_compressed(&mut bytes[..]).unwrap();
                query.commitment.serialize_compressed(&mut bytes[32..]).unwrap();
                query.result.serialize_compressed(&mut bytes[64..]).unwrap();
                hasher.update(bytes);
            }
            hasher.finalize().to_vec()
        }).collect();
        for hashed_result in hash_results.iter() {
            transcript.append_bytes(b"Czy", &hashed_result);
        }

        let duration = start.elapsed();//
        time_vec.push(duration);//time_vec[0]计算hash时间
        let start = Instant::now();
        // 从Transcript对象中挑战一个名为"r"的标量
        let r = transcript.challenge_scalar(b"r");
        // 计算r的幂
        let powers_of_r = powers_of(r, queries.len());
        let duration = start.elapsed();//
        time_vec.push(duration);//time_vec[1]powers_of
        let start = Instant::now();//
        // 将证明者的查询按r的幂进行分组
        let grouped_queries = group_prover_queries(&queries, &powers_of_r);
        let duration = start.elapsed();//
        time_vec.push(duration);//time_vec[2]group_prover_queries
        let start = Instant::now();//
        // 将所有在同一点评估的打开进行聚合
        let aggregated_queries: Vec<_> = grouped_queries
            .into_par_iter() // 将grouped_queries转换为迭代器
            .map(|(point, queries_challenges)| { // 对每个元素应用以下操作
                let mut aggregated_polynomial = vec![Fr::zero(); crs.n]; // 创建一个全为零的向量，长度为crs.n
                // let scaled_lagrange_polynomials: Vec<_> = queries_challenges.into_par_iter().map(|(query, challenge)| {
                //     query.poly.values().iter().map(move |x| *x * challenge)
                // }).collect();
                //
                // for poly_mul_challenge in scaled_lagrange_polynomials {
                //     for (result, scaled_poly) in aggregated_polynomial.iter_mut().zip(poly_mul_challenge) {
                //         *result += scaled_poly;
                //     }
                // }

                queries_challenges.into_iter().for_each(|(query, challenge)| {
                    let poly_mul_challenge = query.poly.values().iter().map(move |x| * x * challenge);
                    for (result, scaled_poly) in aggregated_polynomial.iter_mut().zip(poly_mul_challenge) {
                        *result += scaled_poly;
                    }
                });

                (point, LagrangeBasis::new(aggregated_polynomial)) // 返回点和新的拉格朗日基础
            })
            .collect(); // 收集结果到向量中
        let duration = start.elapsed();//
        time_vec.push(duration);//time_vec[3]进行聚合的时间

        // 计算 g(X)
        let start = Instant::now();//
        let g_x: LagrangeBasis = aggregated_queries
            .iter() // 将aggregated_queries转换为迭代器
            .map(|(point, agg_f_x)| (agg_f_x).divide_by_linear_vanishing(precomp, *point)) // 对每个元素应用以下操作：使用预计算的权重和点来除以线性消失
            .fold(LagrangeBasis::zero(), |mut res, val| { // 使用初始值为零的拉格朗日基础，对每个元素应用以下操作：
                res = res + val; // 将值加到结果上
                res // 返回结果
            });
        let duration = start.elapsed();//
        time_vec.push(duration);//time_vec[4]计算 g(X)
        let start = Instant::now();//

        // 计算 g(X) 的承诺
        let g_x_comm = crs.commit_lagrange_poly(&g_x);
        // 在Transcript对象中添加一个名为"D"的点和它的承诺
        transcript.append_point(b"D", &g_x_comm);
        let duration = start.elapsed();//
        time_vec.push(duration);//time_vec[5]计算 g(X) 的承诺
        let start = Instant::now();//

        // 2.计算 g_1(t)
        let t = transcript.challenge_scalar(b"t"); // 从Transcript对象中挑战一个名为"t"的标量
        let duration = start.elapsed();//
        time_vec.push(duration);////time_vec[5]计算 g1(t) 的承诺
        let start = Instant::now();//

        // 计算 g_1(t) 的分母
        let mut g1_den: Vec<_> = aggregated_queries
            .iter() // 将aggregated_queries转换为迭代器
            .map(|(z_i, _)| t - Fr::from(*z_i as u128)) // 对每个元素应用以下操作：计算 t 减去 z_i
            .collect(); // 收集结果到向量中
        // 批量求逆
        batch_inversion(&mut g1_den);
        let duration = start.elapsed();//
        time_vec.push(duration);
        let start = Instant::now();//

        // 计算 g_1(X)
        let g1_x = aggregated_queries
            .into_iter() // 将aggregated_queries转换为迭代器
            .zip(g1_den) // 将aggregated_queries和g1_den进行配对
            .map(|((_, agg_f_x), den_inv)| { // 对每个元素应用以下操作：
                let term: Vec<_> = agg_f_x
                    .values() // 获取 agg_f_x 的值
                    .iter() // 将值转换为迭代器
                    .map(|coeff| den_inv * coeff) // 对每个元素应用以下操作：计算 den_inv 乘以 coeff
                    .collect(); // 收集结果到向量中

                LagrangeBasis::new(term) // 创建一个新的拉格朗日基础
            })
            .fold(LagrangeBasis::zero(), |mut res, val| { // 使用初始值为零的拉格朗日基础，对每个元素应用以下操作：
                res = res + val; // 将值加到结果上
                res // 返回结果
        ...
        //println!("h(X) time: {:?}", end.duration_since(start));
        let duration = start.elapsed();//
        time_vec.push(duration);
        let start = Instant::now();//
        // 计算 g_1(X) 的承诺
        let g1_comm = crs.commit_lagrange_poly(&g1_x);
        // 在Transcript对象中添加一个名为"E"的点和它的承诺
        transcript.append_point(b"E", &g1_comm);
        ...
        let duration = start.elapsed();//
        time_vec.push(duration);
        let duration = start0.elapsed();//
        time_vec.push(duration);
        //打印time_vec中的所有成员
        // for i in 0..time_vec.len() {
        //     println!("time_vec[{}]: {:?}", i, time_vec[i]);
        // }
        MultiPointProof {
            open_proof: g_3_ipa,
            g_x_comm,
...
impl MultiPointProof {
    ...
    pub fn check_opt(
        ...
        crs: &CRS, // 公共参考字符串
        precomp: &PrecomputedWeights, // 预计算的权重
        queries: &[VerifierQuery], // 验证器查询
        transcript: &mut Transcript, // 会话记录
    ) -> bool {
        // 添加域分隔符
        use rayon::prelude::*;////
        use sha2::{Digest, Sha256};
        ...
        // 检查 IPA
        let b = LagrangeBasis::evaluate_lagrange_coefficients(precomp, crs.n, t); // TODO: 我们可以将这个作为 PrecomputedWeights 的一个方法
...
#[test]
fn multi_proof_large_data_time() {
    ...
    // 创建256个多项式A
    let mut polynomials_a = Vec::new();


    let mut rng = test_rng();
    for _ in 0..400 {
        let poly = LagrangeBasis::new((0..n).map(|_| Fr::rand(&mut rng)).collect());
        polynomials_a.push(poly);
    }


    let mut prover_queries = Vec::new();
    for (_, poly) in polynomials_a.iter().enumerate() {
        let poly_comm_a = crs.commit_lagrange_poly(poly);
        for point in 0..256 {
            let prover_query_a = ProverQuery {
                commitment: poly_comm_a.clone(),
                poly: poly.clone(),
                point,
                result: poly.evaluate_in_domain(point),
            };
            prover_queries.push(prover_query_a);
        }
    }
    ...
    let start = Instant::now();
    let prover_queries1= prover_queries.clone();
    ...
    let start = Instant::now();
    let mut verifier_transcript = Transcript::new(b"test");
    let verifier_queries: Vec<VerifierQuery> = prover_queries.into_iter().map(Into::into).collect();
    assert!(multiproof.check(
        &crs,
        &precomp,
        &verifier_queries,
    ...
    //3000,42s;2000:6s;1000:1.73s

    let duration = start.elapsed();
    println!("clone运行时间: {:?}", duration1);
    println!("MultiPoint::open 运行时间: {:?}", duration2);
    println!("MultiPoint::check 运行时间: {:?}", duration);

}
#[test]
fn multi_proof_large_data_time_optimize() {
    use ark_std::test_rng;
    use ark_std::UniformRand;
    let n = 256;
    let crs = CRS::new(n, b"eth_verkle_oct_2021");
...
#[test]
fn multi_proof_large_data_time_optimize1() {
    use ark_std::test_rng;
    use ark_std::UniformRand;
    use std::time::Instant;
    ...
    let duration = start.elapsed();
    println!("MultiPoint::open 运行时间: {:?}", duration);
